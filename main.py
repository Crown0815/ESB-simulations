...
pot = [-0.9 + 0.001 * x for x in range(1, 2060, 1)]  # potential range
...
fluo = create_coordinates_for_tikz(pot, sim.r_dna * 3, 100e-9, sim.rho_dna * 6)

for r in range(1, 11):
    fluo = sim.normalized_fluorescence(pot, sim.b*sim.bp, sim.r_dna * (1+r*0.1), sim.rho_dna)
